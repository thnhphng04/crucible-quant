...
|---|---|---|---|---|
| O9 | Khi nào chuyển ledger từ SQLite sang Postgres | — | GĐ 2 | Dấu hiệu: nhiều tiến trình ghi đồng thời từ pipeline bất đồng bộ (§3.1.10) gây tranh chấp khóa ở chế độ WAL. P2-08: mỗi slot một kết nối + `busy_timeout`; không tranh chấp với 6 luồng ghi trong test (ADR-0025) |
...
| O14 | Lưới gate ④ = M lần backtest NautilusTrader đầy đủ cho mỗi ứng viên (vài giây mỗi lần) — quá chậm cho vòng lặp GĐ 2 | Thông lượng GĐ 2 | GĐ 2 | Đã thu hẹp bởi [ADR-0025](adr/0025-thong-luong-danh-gia-va-chay-dong-thoi.md): ≈ 5–9 s mỗi backtest sau khi tăng tốc mà kết quả trùng từng bit, máy bão hòa ở ≈ 1 backtest/s với 8–12 slot, nên `M = 200` ≈ 200 s cho mỗi ứng viên tới ④. Đòn bẩy còn lại: `pbo_grid.max_configs` (D13); không dùng backtester vector hoá (P5) |
...
| O19 | `Signal.take_profit` được kiểm tra hợp lệ nhưng không bao giờ được thực thi: bridge NautilusTrader chỉ đặt lệnh stop bảo vệ | Lệnh thoát take-profit trong văn phạm của engine C | GĐ 2+ | Trước khi có, văn phạm không sinh take-profit (một TUNABLE `k_tp` sẽ không đổi gì mà vẫn tốn một tham số); `grammar.GrammarConfig.take_profit_probability = 0` |
...
| O18 | Cổng giai đoạn 1 đòi PBO khớp một ví dụ số đã công bố, điều mà test không dùng | sửa câu chữ kiến trúc §7: PBO khớp fixture tính tay theo định nghĩa CSCV/PBO và một cài đặt độc lập (ghim phiên bản; tính độc lập có test); ví dụ đã công bố có thể bổ sung sau | [ADR-0023](adr/0023-pbo-kiem-chung-bang-dinh-nghia-va-cai-dat-doc-lap.md) |
