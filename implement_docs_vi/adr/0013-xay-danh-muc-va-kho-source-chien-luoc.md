...
## Sửa đổi — review commit b8dc07a

- **Khoảng thời gian của thành viên chỉ thuộc về thành viên** (HIGH). Khoảng giao nhau từng được lấy trên mọi đại diện trước bộ lọc tương quan, và trọng số cùng returns của thành viên được tính trên nó: thêm một ứng viên mà bộ lọc sau đó loại — 100 ngày lịch sử so với 400 ngày của thành viên — đã cắt danh mục còn 100 ngày và đổi Sharpe (4,569 → 5,054), trong khi hash, vốn chỉ phủ thành viên, trọng số và quy tắc, giữ nguyên và artifact đã lưu vẫn 400 ngày. Nay ρ tính theo từng cặp, trên chính những ngày chung của cặp đó (`min_periods = 30`; ít hơn thì ρ không đo được và ứng viên xếp hạng thấp hơn bị loại), còn trọng số và returns chỉ tính trên khoảng giao của các thành viên đã chọn.
- **Cùng hash, cùng dữ liệu.** Khi hash của một lần dựng đã được ghi, returns của nó phải bằng artifact đã lưu, nếu không lần dựng bị từ chối — nên gate ⑤/⑥′ không bao giờ đánh giá dữ liệu khác với artifact của biến thể. Reviewer xác nhận hai biến thể thật không bị ảnh hưởng (artifact và trial thành viên đều 2.819 dòng).
- Test: `tests/validation/test_portfolio.py::test_a_dropped_candidate_does_not_change_the_portfolio`, `::test_member_weights_use_the_members_window_only`, `::test_candidates_with_too_little_overlap_are_not_accepted`, `::test_a_recorded_hash_must_match_its_artifact`.
