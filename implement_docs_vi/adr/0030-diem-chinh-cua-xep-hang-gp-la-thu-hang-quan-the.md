...
- **Đã xác nhận trong vận hành.** Campaign `c-20260924-180916` chạy đủ 600 trial dưới giao thức v4 và cho `gp_beats_random` (hiệu suất 50,33 so với 25,00, spread giữa các seed 5,00). Nghịch lý mà ADR này sinh ra để xử lý đã sụp: trong v2, DSR danh mục của C-gp kém C-random 80 lần; trong v4, ở cùng một ảnh chụp `N`, chỉ còn 1,4 lần (0,0144 so với 0,0201) — trong khi trial efficiency của cả hai arm gần như không đổi. Chỉ tỉ lệ mới so được xuyên campaign, vì `n_eff` đã tăng từ 145 lên 290. `ranking_margin` trên dữ liệu thật đạt 2,72–9,83, so với 0,589 của dạng cũ. Không danh mục nào triển khai được: cả hai thấp hơn `dsr_min` hai bậc độ lớn, đúng như §11 kiến trúc cảnh báo.
