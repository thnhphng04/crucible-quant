...
| INV-81 | Số hạng chính của điểm xếp hạng phân tán hơn tổng các số hạng phụ: IQR(chính) > Σ IQR(phụ) trên mọi quần thể được chấm | §3.1.6 #1, ADR-0030 | Code (+ báo cáo theo (engine, seed) dưới tên `ranking_margin`, không bao giờ là điều kiện dừng) | `tests/agent/evolution/test_ranking.py::test_the_primary_term_out_disperses_the_auxiliary_terms`, `::test_the_absolute_psr_form_fails_the_dispersion_invariant_on_a_compressed_population`, `::test_selection_prefers_the_higher_sharpe_over_a_flatter_neighbourhood`, `tests/agent/test_compare.py::test_the_ranking_margin_is_reported_for_both_arms_and_decides_nothing` | P2-16 | ✅ |
| INV-82 | Hàm xếp hạng của arm nằm trong giao thức đã khóa: λ được hash như dữ liệu, thứ tự lựa chọn làm fingerprint thứ cấp | §3.1.11, ADR-0030 | Code | `tests/agent/test_compare.py::test_the_protocol_hash_covers_the_ranking_lambdas`, `::test_the_ranking_fingerprint_notices_a_reordering`, `::test_the_ranking_scenarios_straddle_the_decision_boundary` | P2-16 | ✅ |
