...
> 🔴 **Đính chính v0.7 — P4 được rút; sizing thành fixed-fractional.** Bản 0.3–0.6 lấy vol targeting làm chân chính và stop chỉ là **trần** (`min`). Quyết định của người dùng ngày 2026-09-25 (ADR-0031) đổi nó thành `Q = R / d`, với `R = max_risk_pct × equity` và `d = stop_distance`. Volatility vẫn đi vào size **đúng một lần**, nhưng qua `stop_distance` (thường k × ATR) chứ không qua chân vol. `target_vol` (D7), IDM và `portfolio_scale` **bị xoá**, không để lại dạng cố vấn. Đổi lại: mỗi vị thế rủi ro **đúng** `max_risk_pct` thay vì **tối đa** `max_risk_pct`, và vol danh mục thành **kết quả** chứ không còn là mục tiêu. Bài học v0.3 vẫn nguyên giá trị và nay dễ giữ hơn: volatility không bao giờ được nhân dồn hai lần — ở dạng mới nó chỉ có đúng một đường vào, là `d`.

```python
class PositionSizer:
    def size(self, signal: Signal, instrument: Instrument,
             account: Account) -> Quantity:
        # 1. Ngân sách rủi ro cố định — số vốn mất khi giá chạm stop
        risk = account.equity * self.max_risk_pct

        # 2. Quy đổi sang số lượng — stop_distance đi vào ĐÚNG MỘT LẦN.
        #    Không còn chân vol targeting, không IDM, không portfolio scale.
        qty = risk / (signal.stop_distance
                      * instrument.multiplier * fx_rate)

        # 3. Đơn vị venue: làm tròn XUỐNG theo lot step, kiểm tra min/max.
        #    Tròn xuống ⇒ rủi ro thực ≤ ngân sách, không bao giờ vượt.
        return round_to_lot(qty, instrument)

# Ở cấp danh mục (§3.2.1): tổng cam kết rủi ro của vị thế mở, lệnh chờ và
# entry mới không vượt max_portfolio_risk_pct của equity, đo tại MỘT snapshot
# dùng chung cho cả batch. q và d đóng băng lúc vào lệnh ⇒ cam kết là hằng số.
committed = sum(q_i * d_i for q_i, d_i in open_positions)
admit = committed + risk <= account.equity * self.max_portfolio_risk_pct
```

Bước **cấp danh mục** không còn là scale đồng đều về `target_vol`. Nó thành một **luật kết nạp**: tại mỗi timestamp, xử lý exit/stop/thanh lý/funding trước, chụp equity một lần, rồi xét entry theo thứ tự canonical; một entry chỉ được nhận nếu tổng cam kết rủi ro vẫn nằm trong trần danh mục. Không nới stop, không tự thêm margin, không đóng vị thế cũ để lấy chỗ.

**Bốn việc tầng này phải làm:**

1. **Ngân sách rủi ro cố định** — mỗi vị thế mất đúng `max_risk_pct` vốn khi chạm stop, không phụ thuộc instrument đó biến động bao nhiêu. Vol danh mục là **kết quả** của rổ, không còn là mục tiêu đặt trước
2. **Trần rủi ro danh mục** — tổng cam kết của vị thế mở, lệnh chờ và entry mới không vượt `max_portfolio_risk_pct`, đo trên một snapshot equity dùng chung cho cả batch
3. **Quy đổi đơn vị** — notional → lot/share/contract theo multiplier từng venue, làm tròn **xuống** để rủi ro thực không bao giờ vượt ngân sách
4. **FX conversion** — hạch toán PnL thống nhất về một đồng tiền base (USD, VND...)

> Kiểm thử bắt buộc: rủi ro tại stop bằng đúng `max_risk_pct × equity` trong khoảng `[R − ε, R]` với ε là phần mất do làm tròn lot/tick; và nhân đôi `vol_estimate` **không** làm đổi size (P4 đã rút), trong khi nhân đôi `stop_distance` làm size giảm đúng ½.
...
| **3** | 🆕 v0.7: **perpetual USDT-M, chiến lược riêng theo `(instrument, direction)`** — hedge mode, isolated margin, một tài khoản chung (ADR-0031). Breadth 15–30 instrument lùi về GĐ 3b | 🔒 Fixture chứng minh đường tài khoản chung: 10 vị thế cùng một snapshot equity, long+short đồng thời trên một contract, funding hai dấu, thanh lý, trần rủi ro 10% chặn entry thứ 11. Preflight dữ liệu thật đạt | 6–10 tuần |
