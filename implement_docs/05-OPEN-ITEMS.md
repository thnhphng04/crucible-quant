...
| O18 | The phase-1 gate asked PBO to match a published numerical example, which the tests do not use | arch §7 reworded: PBO matches hand-computed fixtures from the CSCV/PBO definition and an independent implementation (version pinned; independence tested); a published example may be added later | [ADR-0023](adr/0023-pbo-verified-by-definition-and-an-independent-implementation.md) |
