...
| O18 | The phase-1 gate (arch §7) asks DSR/PBO to match the papers' numerical examples. DSR does; PBO is checked by a hand-computed example from the definition in Bailey et al. (2017) and against `purgedcv` — not against a published numerical example | The PBO part of the phase-1 gate wording | User / phase 2 | Options: find a published worked PBO example to use as a fixture; or accept definition + agreement with an independent implementation for PBO (an arch §7 wording change — the user's call) |
...
| O17 | Should an attempt that failed before anything was measured count in `N`? | no: not a trial, not in `N` or V[SR], no made-up Sharpe; measured-then-rejected still counts; an error after output is classified by a human; traced always; the "each its own cluster" DSR is reported as a sensitivity | [ADR-0022](adr/0022-counting-attempts-that-measured-nothing.md) |
