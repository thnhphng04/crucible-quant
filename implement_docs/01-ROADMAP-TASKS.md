...
- **Remaining:** the run, now under protocol **v4** on a fresh campaign (`compare --lock`, `evolve --engine gp --engine random`, `compare`) — needs Docker, ≈ 15 h at the measured 90 s/trial. Blocked on P2-16: v2's efficiency gap has a known cause in the ranking, so a v3 run would re-measure it.
- **Needs:** P2-13, P2-14.
