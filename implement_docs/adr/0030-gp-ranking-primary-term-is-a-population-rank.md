...
- **Confirmed in production.** Campaign `c-20260924-180916` ran all 600 trials under protocol v4 and returned `gp_beats_random` (efficiency 50.33 against 25.00, seed spread 5.00). The inversion this ADR was written for collapsed: within v2 C-gp's portfolio DSR was 80× C-random's; within v4, at one snapshot of the same `N`, it is 1.4× (0.0144 against 0.0201) — while both arms' trial efficiency barely moved. Only the ratios are comparable across campaigns, since `n_eff` grew from 145 to 290. `ranking_margin` held at 2.72–9.83 on live data, against the old form's 0.589. Neither portfolio is deployable: both sit two orders of magnitude under `dsr_min`, as arch §11 warned.
