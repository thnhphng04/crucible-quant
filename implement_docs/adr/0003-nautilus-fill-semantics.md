...
- **Order size precision = the base currency's** (2026-09-22, first real-data run): instruments used 8 decimals, but Nautilus keeps the XRP balance at 6; selling a whole position left −0.000001 XRP and the engine stopped (`AccountBalanceNegative`), so gate ③ rejected EMA crossover for a harness bug. Each instrument now takes `min(8, base currency precision)`, and the Risk layer rounds to that lot step per symbol. Test: `::test_base_currency_precision_below_the_size_precision`.
