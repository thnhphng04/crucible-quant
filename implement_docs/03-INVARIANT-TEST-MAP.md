...
| INV-41 | PBO ≈ 0.5 on noise, → 0 with a planted edge; a hand-computed example from the paper's definition (not a published numerical fixture, O18) and agreement with `purgedcv` | §3.2 | Code | `tests/validation/test_pbo.py::test_pbo_noise`, `::test_pbo_planted_edge`, `::test_pbo_reference_example`, `::test_matches_purgedcv`; gate: `tests/validation/test_pbo_gate.py` | P1-03 | ✅ |
