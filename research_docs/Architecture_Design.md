...
| **1** | Full validation layer (CPCV/PBO/DSR/MinBTL) + Risk & Sizing | 🔒 The hand-written strategy clears every gate; the ledger separates audit/trials correctly, `N_eff` + `V[SR]` are computable; DSR/PBO match the numerical examples in the original papers; the ½ sizing test (§3.4) passes | 2–3 weeks |
