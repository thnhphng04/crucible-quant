...
5. Mirror it into `implement_docs_vi/adr/` (same number, Vietnamese title in the filename, same line count and headings; template `0000-mau.md`), and the `05` row change into `implement_docs_vi/05-VAN-DE-MO.md`. Run `uv run python scripts/check_doc_mirror.py`.
6. Keep it short: an ADR is read in two minutes. Summarize to the user in Vietnamese.
